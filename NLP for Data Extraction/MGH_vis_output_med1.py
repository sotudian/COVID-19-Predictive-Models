...
def GenBanID():
    ...
    #BanList={'Pharmacologic Substance':0,'Biologically Active Substance':0,'Clinical Drug':0}
    #BanList={'Pharmacologic Substance':0}
    Nonsense = {}
...
out = open('vis_out_med_2.csv', 'a', newline='', encoding='utf-8')
...
for filename in files:
    ...
    for (i, line) in enumerate(lines):
        if started == 0:
            if line[0].lower().find('prior to admission medications') != - \
                    1 or line[0].lower().find('medications prior to admission') != -1:
                started = 1
                n += 1
    ...
    out_line = ['Prior to Admission Medications in visit(1', filename, Time]
    out_line2 = ['COVID19 Medications in visit(1', filename, Time]
    out_line3 = ['Past Medical History inferred by medv(1', filename, Time]
    out_line.extend(D)
