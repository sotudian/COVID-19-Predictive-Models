...
for filename in files:
    ...
    for (i, line) in enumerate(lines):
        if started == 0:
            ...
            if line[0].find('Discharge Medication List') != - \
                    1 or line[0].find('Discharge Medication List'.upper()) != -1:
                started = 1
                n += 1
    ...
    out_line = ['Prior to Admission Medications in visit(7', filename, Time]
    out_line2 = ['COVID19 Medications in visit(7', filename, Time]
    out_line3 = ['Past Medical History inferred by medv(7', filename, Time]
    out_line.extend(D)
