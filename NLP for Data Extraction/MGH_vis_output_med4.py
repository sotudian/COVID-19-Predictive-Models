...
for filename in files:
    ...
    for (i, line) in enumerate(lines):
        if started == 0:
            ...
            if line[0].find('HOME MEDICATIONS') != - \
                    1 or line[0].find('Home Medications') != -1:
                started = 1
                n += 1
    ...
    out_line = ['Prior to Admission Medications in visit(4', filename, Time]
    out_line2 = ['COVID19 Medications in visit(4', filename, Time]
    out_line3 = ['Past Medical History inferred by medv(4', filename, Time]
    out_line.extend(D)
