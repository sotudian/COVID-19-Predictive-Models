...
out = open('vis_out_disease.csv', 'a', newline='', encoding='utf-8')
...
for filename in files:
    ...
    for (i, line) in enumerate(lines):
        if started == 0:
            if line[0].find('Past Medical History') != -1 or line[0].find('PAST MEDICAL HISTORY') != -1 or line[0].find(
                    'Patient Active Problem') != -1 or line[0].find('Past Medical and Surgical History') != -1:
                started = 1
                n += 1
        ...
        if started == 1:
            ...
            # break
        # else:
    out_line = ['Past Medical History in visit', filename, Time]
    out_line.extend(D)
    # ------------- into separate file
