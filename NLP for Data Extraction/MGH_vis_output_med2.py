...
for filename in files:
    ...
    for (i, line) in enumerate(lines):
        if started == 0:
            # if line[0].lower().find('prior to admission medications')!=-1 or
            # line[0].lower().find('medications prior to admission')!=-1:
            if line[0].find('Current Outpatient Medications') != - \
                    1 or line[0].find('Current Outpatient Medications'.upper()) != -1:
                started = 1
                n += 1
    ...
    out_line = ['Prior to Admission Medications in visit(2', filename, Time]
    out_line2 = ['COVID19 Medications in visit(2', filename, Time]
    out_line3 = ['Past Medical History inferred by medv(2', filename, Time]
    out_line.extend(D)
